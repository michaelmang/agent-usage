...
export function buildPlist(cliPath, nodePath = process.execPath) {
...
export function installScheduler() {
    ...
    try {
        execFileSync("launchctl", ["bootout", `gui/${process.getuid?.() ?? 501}/${LAUNCH_AGENT_LABEL}`], {
            stdio: "ignore",
        });
    }
    catch {
...
export function schedulerStatus() {
    ...
    let detail = "";
    try {
        detail = execFileSync("launchctl", ["print", `gui/${process.getuid?.() ?? 501}/${LAUNCH_AGENT_LABEL}`], {
            encoding: "utf8",
