...
test("idempotent usage upsert does not double-count when replacing baseline totals", () => {
    ...
    try {
       ...
       VALUES (?, ?, NULL, NULL, ?)`).run("/tmp/proj", "Proj", now);
        const projectId = db.prepare(`SELECT id FROM projects`).get().id;
        db.prepare(`INSERT INTO sessions(provider_session_id, provider, project_id, cwd, started_at, ended_at, models)
       VALUES (?, ?, ?, ?, NULL, NULL, ?)`).run("sess-1", "claude", projectId, "/tmp/proj", "[]");
        const sessionId = db.prepare(`SELECT id FROM sessions`).get().id;
        const replace = db.prepare(`INSERT INTO usage(
...
test("cumulative delta application only adds positive growth", () => {
    ...
    try {
       ...
       VALUES (?, ?, NULL, NULL, ?)`).run("/tmp/proj", "Proj", now);
        const projectId = db.prepare(`SELECT id FROM projects`).get().id;
        db.prepare(`INSERT INTO sessions(provider_session_id, provider, project_id, cwd, started_at, ended_at, models)
       VALUES (?, ?, ?, ?, NULL, NULL, ?)`).run("sess-1", "codex", projectId, "/tmp/proj", "[]");
        const sessionId = db.prepare(`SELECT id FROM sessions`).get().id;
        db.prepare(`INSERT INTO session_totals(
       ...
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?)`).run(sessionId, "codex", "gpt-5.6-terra", 100, 10, 0, 50, 160, 1.0, now);
        ...
        db.prepare(`INSERT INTO usage(
         session_id, date, provider, model, effort, input_tokens, output_tokens,
         cache_create_tokens, cache_read_tokens, total_tokens, api_equivalent_cost, captured_at
       ) VALUES (?, ?, ?, ?, ?, 0, 0, 0, 0, ?, ?, ?)`).run(sessionId, "2026-08-27", "codex", "gpt-5.6-terra", "", dTotal, dCost, now);
...
test("claude project path encode/decode for kalam-app", async () => {
    const { encodeClaudeProjectPath, decodeClaudeProjectDir } = await import("../ingest/claude-sessions.js");
