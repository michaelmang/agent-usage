...
program
  ...
  .action(async (query: string | undefined, opts) => {
    ...
    const { consolidateCommitMilestones, listMilestones } = await import(
      "./ingest/milestones.js"
    );
    ...
    const rows = opts.commits
      ? consolidateCommitMilestones(rawRows as never)
      : rawRows;
    if (wantJson(opts)) {
    ...
    if (opts.commits) {
      console.log(formatMilestonesTable(rows as never));
      return;
    }
    for (const row of rows as Array<Record<string, unknown>>) {
      ...
      const cost =
        row.apiEquivalentCost != null && Number(row.apiEquivalentCost) > 0
          ? `  ${formatMoney(Number(row.apiEquivalentCost))}`
          : "";
      console.log(
        `${when}  [${row.provider}] ${kind}${sha}${model}${effort}${cost}  ${row.projectName ?? ""}`,
      );
      if (row.gitSubject) console.log(`  ${row.gitSubject}`);
    }
  });
...
program
  ...
  .action(async (periodOrProject: string, projectArg: string | undefined, opts) => {
    await ensureSynced(opts);
    const config = loadConfig();

    if (
      !RECOMMEND_PERIODS.has(periodOrProject) &&
      looksLikeTaskQuery(periodOrProject)
    ) {
      const runtimeOverride =
        opts.runtime === "claude" || opts.runtime === "codex" || opts.runtime === "pi"
          ? opts.runtime
          : undefined;
    ...
    if (RECOMMEND_PERIODS.has(periodOrProject)) {
      period = periodOrProject as "today" | "yesterday" | "week" | "month";
    ...
    let report: ReturnType<typeof buildRecommendReport> | null;
    if (projectQuery) {
      report = buildRecommendReportForProject(db, projectQuery, range.from, range.to, title);
      if (!report) {
        const taskRec = recommendTask(projectQuery);
        if (wantJson(opts)) console.log(JSON.stringify(taskRec, null, 2));
        else console.log(formatTaskRecommendationText(taskRec));
        return;
      }
    } else {
...
program
  ...
  .action(async (opts) => {
    try {
      ...
      if (result.txtPath) console.log(result.txtPath);
      console.log("");
...
program
  ...
  .action(async (opts) => {
    ...
    if (wantJson(opts)) console.log(JSON.stringify(result, null, 2));
      ...
      if (result.reviewMessage) console.log(result.reviewMessage);
...
program
  ...
  .action((opts: { notify?: boolean; review?: boolean; noNotify?: boolean }) => {
    ...
    if (result.review && !process.env.ANTHROPIC_API_KEY) {
      console.log("");
      console.log(
        "Tip: set ANTHROPIC_API_KEY in ~/.config/agent-usage/env for scheduled reviews.",
      );
    }
...
jitCmd
  ...
  .action(async (id: string, opts: { runtime?: string }) => {
    const runtime =
      opts.runtime === "claude" || opts.runtime === "codex" || opts.runtime === "pi"
        ? opts.runtime
        : undefined;
  ...
  .action((opts) => {
    ...
    if (wantJson(opts)) console.log(JSON.stringify(rows, null, 2));
      ...
      for (const row of rows) {
        console.log(
          `${row.id}  ${row.jitLevel}  ${row.spec.runtime.agent}/${row.spec.runtime.model}  ${row.status}  ${row.spec.task.text.slice(0, 60)}`,
        );
      }
    }
  });
...
jitCmd
  ...
  .action(async (task: string | undefined, opts) => {
    ...
    if (wantJson(opts)) {
      console.log(
        JSON.stringify(
          { harnessId: result.harnessId, record: result.record, plan: result.plan },
          null,
          2,
        ),
      );
