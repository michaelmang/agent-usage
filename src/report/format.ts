...
function formatProjectBlock(project: ProjectRollup): string[] {
  ...
  for (const provider of project.providers) {
    ...
    for (const model of provider.models) {
      lines.push(
        `    ${padRight(displayModel(model.model), 18)}${padLeft(formatTokens(model.totalTokens) + " tokens", 14)}  ${padLeft(formatMoney(model.apiEquivalentCost), 10)}`,
      );
    }
  }
  lines.push("  ───────────────────────────────────────────");
...
export function formatModelsTable(
  rows: Array<{ model: string; totalTokens: number; apiEquivalentCost: number }>,
): string {
  const lines = [`${padRight("MODEL", 24)}${padLeft("TOKENS", 12)}${padLeft("API-EQUIV", 12)}`];
  for (const r of rows) {
    lines.push(
      `${padRight(displayModel(r.model), 24)}${padLeft(formatTokens(r.totalTokens), 12)}${padLeft(formatMoney(r.apiEquivalentCost), 12)}`,
    );
  }
  return lines.join("\n");
}
