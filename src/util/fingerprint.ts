...
function walkFiles(root: string, pred: (name: string) => boolean, out: string[] = []): string[] {
...
export function listClaudeSessionFiles(): string[] {
  return walkFiles(CLAUDE_PROJECTS_DIR, (n) => n.endsWith(".jsonl")).filter(
    (f) => !f.includes("/subagents/") && !f.includes("/memory/"),
  );
}
